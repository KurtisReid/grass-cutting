...
//focus on this
        final RestRequest currentUserRequest = oauthRequestTo("https://api.deere.com/platform/boundaries")
                .method("GET")
                .addHeader(new HttpHeader("Accept", V3_ACCEPTABLE_TYPE))
                .build();
...
// change line 47
        final Resource currentUser = read(currentUserResponse).as(Boundaries.class);
        //Boundaries bd = new Boundaries();
        //bd.getMultipolygon().get(0).getRings().get(0).getPoints().get(2);
        //bd.print_long();
        //Boundaries bound = getMultipolygon().
